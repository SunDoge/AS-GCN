...
class InteractionNet(nn.Module):
    ...
    def __init__(self, n_in, n_hid, n_out, do_prob=0., factor=True):
        ...
        self.mlp4 = MLP(n_hid*3, n_hid, n_hid,
                        do_prob) if self.factor else MLP(n_hid*2, n_hid, n_hid, do_prob)
    ...
    # input: [N, v, t, c] = [N, 25, 50, 3]
    def forward(self, inputs, rel_rec, rel_send):
        x = inputs.contiguous()
        # [N, 25, 50, 3] -> [N, 25, 50*3=150]
        x = x.view(inputs.size(0), inputs.size(1), -1)
        # [N, 25, 150] -> [N, 25, n_hid=256] -> [N, 25, n_out=256]
        x = self.mlp1(x)
        # [N, 25, 256] -> [N, 600, 256]|[N, 600, 256]=[N, 600, 512]
        x = self.node2edge(x, rel_rec, rel_send)
        # [N, 600, 512] -> [N, 600, n_hid=256] -> [N, 600, n_out=256]
        x = self.mlp2(x)
        x_skip = x
        if self.factor:
            # [N, 600, 256] -> [N, 25, 256]
            x = self.edge2node(x, rel_rec, rel_send)
            # [N, 25, 256] -> [N, 25, n_hid=256] -> [N, 25, n_out=256]
            x = self.mlp3(x)
            # [N, 25, 256] -> [N, 600, 256]|[N, 600, 256]=[N, 600, 512]
            x = self.node2edge(x, rel_rec, rel_send)
            # [N, 600, 512] -> [N, 600, 512]|[N, 600, 256]=[N, 600, 768]
            x = torch.cat((x, x_skip), dim=2)
            # [N, 600, 768] -> [N, 600, n_hid=256] -> [N, 600, n_out=256]
            x = self.mlp4(x)
        else:
            x = self.mlp3(x)
            x = torch.cat((x, x_skip), dim=2)
...
class InteractionDecoderRecurrent(nn.Module):
    ...
    def single_step_forward(self, inputs, rel_rec, rel_send, rel_type, hidden):
        ...
        all_msgs = torch.zeros(pre_msg.size(
            0), pre_msg.size(1), self.msg_out_shape)
    ...
    def forward(self, data, rel_type, rel_rec, rel_send, pred_steps=1,
        ...
        hidden = torch.zeros(inputs.size(
            0), inputs.size(2), self.msg_out_shape)
        gpu_id = rel_rec.get_device()
...
class AdjacencyLearn(nn.Module):
    ...
    def __init__(self, n_in_enc, n_hid_enc, edge_types, n_in_dec, n_hid_dec, node_num=25):
        ...
        self.rel_rec = torch.FloatTensor(
            np.array(encode_onehot(np.where(off_diag)[1]), dtype=np.float32))
        self.rel_send = torch.FloatTensor(
    ...
    def forward(self, inputs):  # [N, 3, 50, 25, 2]
        ...
        x = x.contiguous().view(N*M, V, C, T).permute(0,
                                                      1, 3, 2)  # [2N, 25, 50, 3]
        ...
        self.outputs = self.decoder(
            x, self.edges, rel_rec, rel_send, burn_in=False, burn_in_steps=40)
        ...
        for i in range(self.N):
            ...
            for j in range(1, self.c):
                A = torch.sparse.FloatTensor(self.offdiag_indices, self.edges[i, :, j], torch.Size(
                    [25, 25])).to_dense().cuda(gpu_id)
